...
namespace PaginaIst.Servicesrentados
    ...
    /// </summary>
    public class ReporteEquipoServicerentados : IReporteEquipoServicerentados
    {
        private readonly IWebHostEnvironment _env;

        // 👇 Inyectamos IWebHostEnvironment para leer el logo desde wwwroot
        public ReporteEquipoServicerentados ( IWebHostEnvironment env )
            {
            _env = env;
            }
        ...
        // =====================================================
        public byte [] GenerarPdfEquipo ( EquiposRentados equiporentados )
            {
            ...
            var document = Document.Create(container =>
            {
                container.Page(page =>
                {
                    ...
                    page.Header().Element(c =>
                        DibujarEncabezado(c, logoData, $"HOJA DE VIDA EQUIPO - {equiporentados.Placa}"));

                    // 🔹 Contenido detallado del equipo
                    page.Content().PaddingVertical(10).Column(col =>
                    {
                        col.Item().Text($"ID: {equiporentados.id}");
                        col.Item().Text($"PLACA: {equiporentados.Placa}");
                        col.Item().Text($"ID_EMPLEADO: {equiporentados.Id_Empleado}");
                        col.Item().Text($"HOSTNAME: {equiporentados.Hostname}");
                        col.Item().Text($"FECHA_INICIAL: {equiporentados.Fecha_Inicial:dd/MM/yyyy}");
                        col.Item().Text($"FECHA_FINAL: {equiporentados.Fecha_Final:dd/MM/yyyy}");
                        col.Item().Text($"TIPO EQUIPO: {equiporentados.Id_Tipoequipo}");
                        col.Item().Text($"MARCA: {equiporentados.Marca}");
                        col.Item().Text($"MODELO: {equiporentados.Modelo}");
                        col.Item().Text($"SERIAL: {equiporentados.Serial}");
                        col.Item().Text($"NIT PROVEEDOR: {equiporentados.Nit_Proveedor}");
                        col.Item().Text($"GARANTÍA (años): {equiporentados.Garantia}");
                        col.Item().Text($"FUENTE/CARGADOR: {equiporentados.Fuente}");
                        col.Item().Text($"CAPACIDAD FUENTE: {equiporentados.Capacidad_Fuente}");
                        col.Item().Text($"PROCESADOR: {equiporentados.Procesador}");
                        col.Item().Text($"CLASE DISCO N°1: {equiporentados.Clase_DiscoN1}");
                        col.Item().Text($"CAPACIDAD DISCO N°1: {equiporentados.Capacidad_Disco_N1} GB");
                        col.Item().Text($"CLASE DISCO N°2: {equiporentados.Clase_Disco_N2}");
                        col.Item().Text($"CAPACIDAD DISCO N°2: {equiporentados.Capacidad_Disco_N2} GB");
                        col.Item().Text($"RAM N°1: {equiporentados.MEMORIA_RAM_N1} GB");
                        col.Item().Text($"RAM N°2: {equiporentados.MEMORIA_RAM_N2} GB");
        ...
        // =====================================================
        public byte [] GenerarPdfListado ( IEnumerable<EquiposRentados> equiposrentados )
            {
            var lista = equiposrentados.ToList();
